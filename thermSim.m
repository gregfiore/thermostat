function [t, heater_status, room_temp, c_actual, c_estimated] = thermSim(varargin)
%
% FUNCTION thermSim
% 
% Description:
...
% Inputs (optional):
%   
%     1.  time:           vector of time in hours for simulation
%     2.  setpoint:       thermostat setpoint (as a function of time)
%     3.  outdoor_temp:   outside temperature in degrees F (as a function of time)
%     4.  learning_mode:  'On' or 'Off'
%     5.  print_debug:    flag (1 or 0) for printing model data to screen
%     6.  plot_debug:     flag (1 or 0) for plots of simulation results
%     7.  Req_sf:         scale factor to scale house equilalent thermal resistantce (for testing purposes)
%     8.  THeater_sf:     scale factot to scale heater air temperature (for testing purposes) 
%
%     NOTE:  If no inputs are provided, the simualtion will run in default mode
%   
% Outputs:  
% 
%     1.  t:              time vector returned from simulation data
%     2.  heater_status:  heater on/off status during simulation
%     3.  room_temp:      house temperature during simluation
%     4.  c_actual:       actual thermal parameters of model
%     5.  c_estimated:    estimated thermal parameters of model (from learning mode)
% 
...
switch nargin
    ...
    otherwise 
        time = varargin{1};
        setpoint = varargin{2};
        outdoor_temp = varargin{3};
        learning_mode = varargin{4};
        print_debug = varargin{5};
        plot_debug = varargin{6};
        
        if nargin > 6
            Req_sf = varargin{7};
            THeater_sf = varargin{8};
        end
        
end

assignin('base','setpoint',setpoint)
assignin('base','outdoor_temp',outdoor_temp)
...
temp_threshold = 0.5;     % Change in room temperature threshold for when heater effects first occur [dec. C]
...
% Set Simulink parameters based on simulation
set_param('therm_model2/Heater/Heater Air Temp','Value',num2str(THeater));
set_param('therm_model2/Heater/Heat Gain','Gain',num2str(Mdot*c));
set_param('therm_model2/House/Gain','Gain',num2str(1/(M*c)));
set_param('therm_model2/House/Heat Loss','Gain',num2str(1/Req));
set_param('therm_model2/House/Integrator','InitialCondition',num2str(TinIC));
set_param('therm_model2/Thermostat/On Delay','Time',num2str(on_delay));
set_param('therm_model2/Thermostat/Off Delay','Time',num2str(off_delay));
...
switch learning_mode
    case 'On'
        ...
        if plot_debug
            ...
            set(gca,'XLim',[on_delay - 0.5*(off_delay-on_delay), t(last_id) + 0.5*(off_delay-on_delay)])
            title('System Identification Window')
            ylabel('House Temp [deg. C]')
